...
module.exports = {
    ...
    metadata: () => ({
        ...
        "properties": {
            ...
            "highestSurplusAccount": {
                "type": "string",
                "required": false
            },
            "highestSurplusAmount": {
                "type": "string",
                "required": false
            }
    ...
    invoke: (conversation, done) => {
        ...
        const highestSurplusAccount = conversation.properties().highestSurplusAccount;
        const highestSurplusAmount = conversation.properties().highestSurplusAmount;
        ...
        soap.createClient(url, function(err, client) {
            ...
            client.executeSQLQuery(args, function(err, result) {
                ...
                parseString(resp, {
                    ...
                    //get the object with the most budget available. The values come from OTBI API sorted by (budget - actual)
                    //so the last item will have the most budget remaining:
                    var maxFundsAvailable = arr[arr.length - 1];
                    var account = maxFundsAvailable.Column4;
                    var accountDesc = account.substr(account.indexOf('-')+ 1);
                    conversation.variable("highestSurplusAccount", accountDesc);
                    conversation.variable("highestSurplusAmount", formatter.format(parseInt(maxFundsAvailable.Column11)));
